...
const ENVIRONMENT = process.env.MKOPAJI4000_ENVIRONMENT || 'sandbox';

console.log(`🚀 Starting MKOPAJI-4000 Server in ${ENVIRONMENT.toUpperCase()} mode`);
    ...
    try {
        ...
        if (result.success) {            
            ...
            transactions.set(result.CheckoutRequestID, {
                ...
                status: 'pending',
                timestamp: new Date(),
                provider: result.provider || 'mkopaji-4000',
                applicationData
            });
...
app.get('/api/check-transaction-status/:checkoutRequestID', async (req, res) => {
    try {
        ...
        if (statusResult.success) {
            ...
            if (statusResult.mkopaji4000ReceiptNumber) {
                transaction.mkopaji4000ReceiptNumber = statusResult.mkopaji4000ReceiptNumber;
            }
            ...
            res.json({
                success: true,
                status: statusResult.status,
                mkopaji4000ReceiptNumber: statusResult.mkopaji4000ReceiptNumber,
                transaction: {
...
app.post('/api/confirm-payment', async (req, res) => {
    try {
        const { checkoutRequestID, mkopaji4000Code } = req.body;
        ...
        if (mkopaji4000Code) {
            transaction.mkopaji4000ReceiptNumber = mkopaji4000Code;
        }
        transactions.set(checkoutRequestID, transaction);

        console.log('✅ Payment manually confirmed:', {
            checkoutRequestID,
            mkopaji4000ReceiptNumber: mkopaji4000Code,
            amount: transaction.amount
        });
...
app.post('/api/simulate-payment-success', async (req, res) => {
    try {
        ...
        transaction.status = 'completed';
        transaction.mkopaji4000ReceiptNumber = 'QEJ7Y6X2M1'; // Sample mkopaji-4000 code
        transactions.set(checkoutRequestID, transaction);

        console.log('🧪 TEST: Payment simulated as successful:', {
            checkoutRequestID,
            mkopaji4000ReceiptNumber: transaction.mkopaji4000ReceiptNumber,
            amount: transaction.amount
        });
...
app.post('/api/mkopaji-4000-callback', async (req, res) => {
    ...
    try {
        ...
        if (ResultCode === 0) {
            ...
            if (callbackMetadata && callbackMetadata.Item) {
                for (const item of callbackMetadata.Item) {
                    switch (item.Name) {
                        case 'Mkopaji4000ReceiptNumber':
                            transaction.mkopaji4000ReceiptNumber = item.Value;
        ...
        console.log('📞 📊 Final transaction state:', {
            ...
            mkopaji4000ReceiptNumber: transaction.mkopaji4000ReceiptNumber || 'N/A'
        });
...
app.post('/api/mkopaji-4000-timeout', (req, res) => {
    console.log('⏰ mkopaji-4000 Timeout Callback received:', JSON.stringify(req.body, null, 2));
    res.status(200).json({ ResultCode: 0, ResultDesc: "Timeout acknowledged" });
});

// mkopaji-4000 Result Callback (for B2C and other operations)
app.post('/api/mkopaji-4000-result', (req, res) => {
    console.log('📋 mkopaji-4000 Result Callback received:', JSON.stringify(req.body, null, 2));
...
async function processLoanDisbursement(applicationData, transaction) {
    try {
        ...
        // await mkopaji4000Service.b2cPayment(
...
// Start server
app.listen(PORT, () => {
    console.log(`🚀 MKOPAJI-4000 Loan Server running on port ${PORT}`);
