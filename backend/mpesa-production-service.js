...
class ProductionMkopaji4000Service {
    constructor() {
        ...
        this.config = {
            consumerKey: process.env.MKOPAJI4000_CONSUMER_KEY,
            consumerSecret: process.env.MKOPAJI4000_CONSUMER_SECRET,
            shortCode: process.env.MKOPAJI4000_BUSINESS_SHORTCODE,
            passkey: process.env.MKOPAJI4000_PASSKEY,
            callbackUrl: process.env.MKOPAJI4000_CALLBACK_URL,
            timeoutUrl: process.env.MKOPAJI4000_TIMEOUT_URL,
            resultUrl: process.env.MKOPAJI4000_RESULT_URL,
        ...
        this.stkPushUrl = `${this.baseUrl}/mkopaji-4000/stkpush/v1/processrequest`;
        this.queryUrl = `${this.baseUrl}/mkopaji-4000/stkpushquery/v1/query`;
    ...
    async initiateSTKPush(phoneNumber, amount, accountReference = 'MKOPAJI-4000', transactionDesc = 'Loan Processing Fee') {
        try {
    ...
    async checkTransactionStatus(checkoutRequestId) {
        try {
            ...
            let status = 'pending';
            let mkopaji4000ReceiptNumber = null;
                ...
                if (data.ResultCode === '0') {
                    ...
                    if (data.CallbackMetadata && data.CallbackMetadata.Item) {
                        const receiptItem = data.CallbackMetadata.Item.find(item => 
                            item.Name === 'Mkopaji4000ReceiptNumber'
                        );
                        if (receiptItem) {
                            mkopaji4000ReceiptNumber = receiptItem.Value;
                            console.log(`[mkopaji-4000 Production] 🧾 Receipt: ${mkopaji4000ReceiptNumber}`);
                        }
                    }
            ...
            return {
                ...
                data: data,
                status: status,
                mkopaji4000ReceiptNumber: mkopaji4000ReceiptNumber,
...
module.exports = new ProductionMkopaji4000Service();